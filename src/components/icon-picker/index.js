...
const NativeIconPicker = ({
    ...
    return (
        <>
            ...
            />
            <BaseControl id="hang-icon-settings" label={label} __nextHasNoMarginBottom>
                <Dropdown
                    popoverProps={{
                    ...
                    className="hang-icon-settings"
                    renderToggle={({ isOpen, onToggle, onClose }) => (
                        <div className="hang-icon-settings__dropdown">
                            <Button
                                onClick={onToggle}
                                aria-expanded={isOpen}
                                className="hang-icon-settings__dropdown-toggle"
                                __next40pxDefaultSize
                                __nextHasNoMarginBottom
                            >
                                {!!(iconName || customSvgCode) ? (
                                    <span className="hang-icon-settings__indicator">
                                        <RenderIcon customSvgCode={customSvgCode} iconName={iconName} size={20} />
                                    </span>
                                ) : (
                                    <span className="hang-icon-settings__indicator disabled"></span>
                                )}

                                <span className="hang-icon-settings__dropdown-label">
                                    {__(iconName || (customSvgCode ? 'Custom SVG' : 'Select Icon', 'han-group'))}
                                </span>
                            </Button>
                            <Button
                                label={__('Browse library', 'han-group')}
                                onClick={() => {
                                    onClose();
                                    openModal('library');
                                }}
                                ...
                                size="small"
                                icon={folderOpen}
                                className="hang-icon-settings__dropdown-more"
                                __next40pxDefaultSize
                                __nextHasNoMarginBottom
                            />
                        </div>
