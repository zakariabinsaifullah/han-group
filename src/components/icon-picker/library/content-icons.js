    ...
    return (
        <>
            <Flex>
                <FlexItem>
                    <SearchControl
                        ...
                        label={__('Search icons', 'han-group')}
            ...
            {filteredIcons.length === 0 ? (
                <p>{__('No icons found!', 'han-group')}</p>
            ) : (
                <div className="hang-modal__icons">
                    {filteredIcons.map(iconData => (
                        <Button
                            key={iconData.name}
                            className={`hang-modal__icons-button ${currentIconName === iconData.name ? 'is-selected' : ''}`}
