 ...
 */
addFilter( 'blocks.registerBlockType', 'hang/group-overlay-bg-add-attributes', ( settings, name ) => {
    if ( name !== BLOCK_NAME ) {
        return settings;
    }
...
addFilter(
    'editor.BlockEdit',
    'hang/group-overlay-bg-add-inspector-controls',
...
/**
 * Apply `hang-overlay-bg` class and `--hang-overlay-bg` CSS variable in the editor preview.
 */
addFilter(
    'editor.BlockListBlock',
    'hang/group-overlay-bg-add-styles',
        ...
        return props => {
            ...
            const wrapperProps = {
                ...
                style: {
                    ...
                    '--hang-overlay-bg': bgValue
                }
            };

            const classes = [ props.className, 'hang-overlay-bg' ].filter( Boolean ).join( ' ' );
